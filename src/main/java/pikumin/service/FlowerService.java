...
public class FlowerService {
    ...
    public void setSeedToGrow(String username, Long seedId) {
        ...
        List<Seed> growingSeeds = seedRepository.findByOwnerAndGrowingTrue(user);
        if (growingSeeds.size() >= 3) {
            throw new RuntimeException("育成中の種は最大3つまでです。");
        }

        Seed seed = seedRepository.findByIdAndOwner(seedId, user);
        if (seed == null) {
